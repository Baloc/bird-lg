...
def bird_socket_singleton(host, port):
    global bird_sockets
    sock = bird_sockets.get((host, port), None)
    if not sock:
        sock = BirdSocket(host, port)
        bird_sockets[(host, port)] = sock
    return sock
...
class BirdSocket:
    ...
    def cmd(self, cmd):
        try:
            ...
            self.close()
            return False, "Bird connection problem: {reason}".format(reason=reason)
...
__all__ = ['bird_socket_singleton', 'BirdSocket']
